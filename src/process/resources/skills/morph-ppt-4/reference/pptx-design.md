...
### Practical Variants (recommended)

Use these as first-choice variants before inventing complex new layouts:

| Variant               | Best For                          | Structure Hint                                                     |
| --------------------- | --------------------------------- | ------------------------------------------------------------------ |
| **dual-evidence**     | 2 key metrics / before-after      | Two large stat blocks + one shared conclusion line                |
| **triad-spotlight**   | 3 parallel points                 | 3 cards, one highlighted, others subdued                          |
| **problem-solution**  | obstacle -> fix                   | Left pain point / right solution                                  |
| **myth-fact**         | misconception correction          | Short myth strip + stronger fact block                            |
| **principle-action**  | strategy to execution bridge      | Top principle row + bottom action checklist                       |
| **checklist-cta**     | closing and mobilization          | 3 action bullets + one commitment statement                       |
| **quote-evidence**    | emotional claim with proof        | Large quote + small supporting metric/data chip                   |
| **showcase-with-callout** | product/demo explanation      | Main visual area + 2-3 callout labels                             |

### Timeline Fallback Rule (important)

`timeline` often looks weak when step content is short. Use this rule:

- Keep `timeline` only when all are true:
  - clear sequence dependency exists
  - at least 4 steps
  - each step has meaningful detail (not just 2-4 words)
- If any condition fails, downgrade to:
  - `pillars` (parallel steps),
  - or `evidence` (stage outcome focus),
  - or `comparison` (before vs after stage).

### Anti-Patterns to Avoid

- Big containers with tiny text content.
- Repeating identical card grids on consecutive slides.
- Timeline with decorative arrows but no real stage detail.
- More than one dominant focal block in a single slide.
