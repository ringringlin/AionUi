...
- ✅ Container-content density is balanced (avoid large empty cards)
- ✅ Typography size floors are respected by text role
- ✅ For concrete topics, theme-anchor coverage is sufficient (adaptive rule)
- ✅ Topic-binding is explicit (avoid generic decorative-only pages)
- ✅ Component borrowing is non-clone (structure adapted, not copied)

### Readability-First (adaptive reference rules)

Use this sequence as default behavior (flexible first, strict fallback):

1. Flex adjustment first:
   - Increase text box width or move nearby decorative elements.
   - Raise body font size by `+1~2pt` in dark/muted scenes.
   - Reduce decoration prominence (`L4` then `L2`) before touching core text.
2. Soft threshold check:
   - If text is visually crowded, reduce card height mismatch and simplify nearby shapes.
   - If reading path is unclear, keep one dominant text focal point and weaken secondary accents.
3. Strict fallback only when still unreadable:
   - Remove conflicting decoration blocks.
   - Re-layout text blocks to avoid overlap.
   - Keep content legible even if visual richness is reduced.
...
| Large container with tiny content | Main card height far larger than text content               | Shrink card height, merge cards, or upgrade content hierarchy                      |
| Body text too small               | Core paragraph text uses `font<=14`                         | Raise body to 16-22; keep 14-16 only for short captions                            |
| Text readability conflict         | Decoration competes with title/body                         | Apply adaptive sequence: widen text -> raise size -> weaken decoration -> relayout |
| Concrete topic not recognizable   | Slides look generic; subject cannot be identified quickly   | Add lightweight recognizable theme anchors and increase deck-level anchor coverage |
| Component looks copied            | Same block order, same spacing rhythm as one sample         | Change at least two dimensions: order/spacing/focus/actor path before delivery     |
| Generic motif overuse             | Cross + circle + orbit dominates consecutive slides         | Keep motif as background only; switch composition rhythm and focal hierarchy       |
...
- ✅ After each clone, previous slide content (`#sN-`) is fully ghosted to `x=36cm`
...
| Previous-slide content accumulates           | Ghost parser mismatch or naming mismatch                                                                                                                                                   | Use helper canonical parser (`children/format/path`), then fail-fast if matched_count=0 or any `x!=36cm` |
...
3. **Density**: Any obvious “big container + tiny content” mismatch?
4. **Typography**: Body text at readability floor? (`body >=16`, caption-only can be 14-16)
5. **Theme**: If topic is concrete, does this slide contribute to recognizable theme anchors?
6. **Morph**: Scene actors have same names as previous slide? Scene actors not needed are ghosted to `x=36cm`?
7. **⚠️ TEXT OVERLAP CHECK**: Previous slide's content actors are ALL ghosted (`x=36cm`)? This slide's new content is added fresh?
8. **BORROW-NOT-CLONE CHECK**: Is this slide clearly adapted to topic intent (not sample skeleton replay)?

Note:

- Prefer flexible correction first. Strict removals are the last resort.
- Readability is the top priority for final delivery.

Text overlap reminder:
