...
# OfficeCli Remote Style + Component Source (5.0)

This guide defines how to use OfficeCli `Styles/` as the remote source of truth, with a description-first non-clone component borrowing strategy.
...
Morph PPT 5.0 keeps a local baseline copy of component docs:
...
Morph PPT 5.0 adds one stricter rule:

- Remote fetch provides inspiration and decision evidence only.
- Final layout must be re-composed by slide intent; never replay a component sample structure as-is.
...
# Do not fetch build.sh by default in 5.0
...
# Do not fetch components.py by default in 5.0
