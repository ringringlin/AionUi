  ...
  class DeckStage extends HTMLElement {
    ...
    _applyIndex({ showOverlay = true, broadcast = true, reason = 'init' } = {}) {
      ...
      if (broadcast) {
        ...
        try {
          window.postMessage(
            { slideIndexChanged: curr, deckTotal: this._slides.length, deckSkipped: this._skippedIndices() },
            '*'
          );
        } catch (e) {}
    ...
    _materialize(entry) {
      ...
      const neuter = (el) => {
        ...
        box.style.cssText =
          (el.getAttribute('style') || '') + ';background:rgba(0,0,0,0.06);border:1px dashed rgba(0,0,0,0.15);';
    ...
    _toggleSkip(i) {
      ...
      try {
        window.postMessage(
          { slideIndexChanged: this._index, deckTotal: this._slides.length, deckSkipped: this._skippedIndices() },
          '*'
        );
      } catch (e) {}
    }
